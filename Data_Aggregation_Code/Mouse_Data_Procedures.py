'''
Code to calculate mouse usage parameters per mouse task per participant (and visualize the data).

THIS IS A PROCEDURE THAT IS USED TOGETHER WITH THE aggregated_dataset_creation.py file

There is a function to calculate the mouse features per task
...
########################################
# Functions to preprocess the mouse data
########################################
...
def clean_mouse_data(mouse_data):
    ...
    # Sort the page data by timestamp
...
def interpolate_mouse_data(mouse_data):
    ...
    # Seperately save the x- & y-coordinate as well as the timestamp of each datapoint
    x_pos = []
    y_pos = []
    time = []
...
###################################################
# Functions to calculate the mouse usage parameters
###################################################
...
def get_mouse_task_time(mouse_data):
    ...
    # Get the first and last timestamp of the mouse usage during the task
...
def get_mouse_movement_parameters(mouse_data):
    ...
    # time constant between two datapoints (corresponds to the interpolate value)
    interpol = 15
    ...
    for datapoint in mouse_data:
        if last_x and last_y:
            ...
            # calculate the euclidean distance between the two datapoints
...
# this function used mouse data split into trials, because the ideal lines are between the trial start point
# and trial end point
def get_dist_from_ideal_line(mouse_data):
    ...
    for trial in mouse_data:
        ...
        if interpol_trial != -99:
            ...
            # loop over all data points from a trial
...
# calculates the angle between two consecutive mouse movement vectors (between three mouse movement datapoints)
...
# entropy is not used as a mouse parameter in the manuscript
...
###################################################################################################################
# Function to visualize mouse movement and get images of the mouse movements that are used for image classification
###################################################################################################################

def visualize_mouse_movement(data, participant, task):
    ...
    # plt.savefig('path/' + task + "_" + str(participant) + '.png', bbox_inches="tight")

    plt.close()


#####################################################
# Function to calculate the mouse parameters per task
#####################################################


# Point & Click Task:
# The task starts with the beginning of the first successfull click until the last click
def get_point_click_data(dataset):

    # get the task name of the dataset
    ...
    # participants that did not complete the task (not used at the moment, but could be to filter bad cases)
    ...
    for participant in dataset:
        ...
        for task in pc_task:
            ...
            # create a row per condition -> task[:2] = "HS" or "LS"
            ...
            if task in dataset[participant]:
                ...
                # only consider the data after the first successful mouseclick to the last mouseclick
                ...
                ######
                # End of code to create a dictionary with the raw eda data per participant per phase
                ######
                ...
                # print information about participants who did not finish the task
                # if len(trial_data) < 25:
                #     print(participant + " has completed " + str(len(trial_data)) + " trials in " + task)

                ######
                # Calculate the point and click task mouse usage parameters
                ######
    ...
    # per participant containing all point and click task mouse usage features
...
# Drag & Drop Task:
# The task starts with the beginning of the first first drag
def get_drag_drop_data(dataset):
    ...
    for participant in dataset:
        ...
        for task in dd_task:
            ...
            if task in dataset[participant]:
                ...
                # Creates a list that contains trial data for successfull drag & drop operations split into
                # the part where the user drags the circle and the part where the user navigates back
                # to the dragging circle
    ...
    # save the final results data as a pandas dataframe and return the dataframe, it contains a row per condition
    # per participant containing all drag and drop task mouse usage features
    final_data = pd.DataFrame(results).T
...
# Follow Box Task:
# starts as soon as the mouse cursor moves inside the task box and the task box starts moving
def get_follow_box_task_data(dataset):
    ...
    # there are no bad cases in this task
    ...
    # save the final results data as a pandas dataframe and return the dataframe, it contains a row per condition
    # per participant containing all follow box task mouse usage features
    final_data = pd.DataFrame(results).T
...
# Drawing Task:
# starts as soon as the the first drawing begins
def get_steering_task_data(dataset):
    ...
    # save the final results data as a pandas dataframe and return the dataframe, it contains a row per condition
    # per participant containing drawing task mouse usage features
    final_data = pd.DataFrame(results).T
    print("Steering Task Data Successfully Processed!")
    return final_data


# Data from participant's mouse movement during the loading screen
# !! Not part of the analysis in the manuscript !!
def get_loading_task_data(dataset):
    ...
    # save the final results data as a pandas dataframe and return the dataframe, it contains a row per condition
    # per participant containing loading task mouse usage features
    final_data = pd.DataFrame(results).T
