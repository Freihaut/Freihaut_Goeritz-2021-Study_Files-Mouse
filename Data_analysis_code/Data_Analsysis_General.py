...
of the data, many more approaches were done which are not part of this file but can be requested by the author. This
include different tests (e.g. test for equal variance, regression approaches instead of classification,
classification and regression using the questionnaire data and physiological data ...) or different data preprocessing
strtegies such as principal compenent analysis etc. The results of all other approaches did not reveal any significant
relationship between mouse usage and stress or stress markers.

To excecute the code, make sure that the correct datasets are used and that they are corectly imported. The functions
that excetute the analysis procedures can be found at the end of the file
...
def manipulation_check_data_t_test(data_source):
    ...
    # get the dataset
    dataset = pd.read_csv("../Datasets/Manipulation_Check_Data.csv", sep='\t', encoding='utf-8')
...
def mouse_data_t_test(task):
    ...
    # get the dataset (here aggregated_mouse_data.csv
    dataset = pd.read_csv("../Datasets/Aggregated_Mouse_Data.csv", sep='\t', encoding='utf-8')
...
def paired_sample_t_test(dataframe):
    ...
    # print(hs_frame.shape)
    # print(ls_frame.shape)
    ...
    with open("aae.txt","a") as f:
        ...
        for i in range(len(dataframe.columns)):
            ...
            results[i] = {}
            results[i]["HS_mean"] = np.round(np.mean(hs_frame.iloc[:, i]), 2)
            results[i]["HS_std"] = np.round(np.std(hs_frame.iloc[:, i]), 2)
            results[i]["LS_mean"] = np.round(np.mean(ls_frame.iloc[:, i]), 2)
            results[i]["LS_std"] = np.round(np.std(ls_frame.iloc[:, i]), 2)
            results[i]["t-value"] = np.round(stats.ttest_rel(hs_frame.iloc[:, i], ls_frame.iloc[:, i])[0], 2)
            results[i]["p-value"] = np.round(stats.ttest_rel(hs_frame.iloc[:, i], ls_frame.iloc[:, i])[1], 3)
    ...
    final_data.to_csv(r"t-test_results.csv", sep='\t', encoding='utf-8')


# The Manipulation Check Data can be analysed using the paired_sample_t_test procedure aswell, our manipulation check
# analysis was done with SPSS (was done before the creation of the python data analysis scripts)
# To reprocude the results using this approach the following steps are necessary:
# 1. create a dataset containing the manipulation check items (MDBF, SAM, Heart Rate and EDA)
# 2. prepare the data similar to the mouse_data_t_test function
# 3. use the paired_sample_t_test function to get and save the results
...
# function to create the dataset for the task condition classification
...
def ml_bootstrap(task, dv, iv, groups):
    ...
    def classifier_feature_selection(dv, x_train, y_train, x_test, method="all", threshold=0.05):
        ...
        # Wrapper Methods: Search for an "optimal ammount of features" using a selection process (adding the feature
        # that most improves accuracy until no feature addition improves accuracy == Foward Selection)
        # Using the mlxtend library ("stepwise selection procedure using k-fold-cv)
        if method == "wrapper":
...
# print the machine learning results
# arguments are the dataset creation function and a task array -> prints every task of the task array using the
# specific data from the datafunction, eg. ml_keyboard_mouse_data
def get_results(datafunction, tasks):
    for task in tasks:

        # condition classification
        # dv, iv, groups = datafunction(task)
        # print(dv.index)
        # ml_permutation_test(task, dv, iv, groups)
        # ml_bootstrap(task, dv, iv, groups)

        # t-test
        dv = datafunction(task)
        paired_sample_t_test(dv)


# choose the tasks that are analysed
tasks = ["PointClick", "DragDrop", "Drawing", "FollowBox"]
tasks = ["Drawing"]
# chose the data type that is analysed using the manipulation check data
data_sources = ["heart_data", "eda_data", "questionnaire"]

# choose a datafunction corresponding to either machine learning or t-test
# machine_learning = ml_mouse_data
# t-test = mouse_data_t_test or manipulation_check_data_t_test

# call the function to get the results
# (in this example, the condition is classified on all tasks using bootstrap
get_results(mouse_data_t_test, tasks)
